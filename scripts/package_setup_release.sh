...
#
# Writes: dist/tenchu-stealth-assassins-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix tenchu-stealth-assassins-recomp \
  --exe-name Tenchu_Stealth_Assassins_Recompiled \
  --display-name "Tenchu: Stealth Assassins Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Tenchu Stealth Assassins disc" \
